...
#makes a folder with the appropiate separation of images basedd on the labels
def makeFolder(df,folder_path):
    
    destination = 'Resources/data'
   
    destination_positive = f'Resources/data/{df.columns[1]}'
    destination_negative = f'Resources/data/No {df.columns[1]}'
    
     # Create the new directory
    os.makedirs(destination_positive, exist_ok=True)
    # Create the new directory
    os.makedirs(destination_negative, exist_ok=True) 
    for file in os.listdir(folder_path):
        ...
        if not data.empty:  # Check if data is not empty
            ...
            if label == 'Yes':
                destination_path = os.path.join(destination_positive, file)
                source_path = os.path.join(folder_path, Filename)
                shutil.copy(source_path, destination_path)
            else:
                destination_path = os.path.join(destination_negative, file)
                source_path = os.path.join(folder_path, Filename)
                shutil.copy(source_path, destination_path)
